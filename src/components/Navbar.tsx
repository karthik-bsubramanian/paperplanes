...
export const Navbar = ({ handleOnClick, publishing }: Props) => {
  ...
  const currentUserId = jwtDecode<decodeJWT>(localStorage.getItem('authtoken') as string);
  ...
  return (
    <div className="flex h-11 sticky top-0 items-center bg-sandal text-green font-normal text-sm justify-between z-50">
      ...
      <div className="flex mr-5 items-center">
        ...
        >
          <img src={userData?.image} alt="er" className="rounded-full object-cover" />
          {isOpen && (
            <div className="absolute top-10 right-0 bg-sandal w-40 h-20 shadow-lg px-2 pt-2">
              <div className=" mx-auto text-green-800 hover:text-green mb-2">
                <button
                  onClick={() => navigate(`/profile?id=${currentUserId.id}`)}
                  className="flex gap-2 text-sm font-semibold cursor-pointer"
                >
                <CircleUserRound className="h-5 w-5" strokeWidth={0.5} />
                  Profile
                </button>
              </div>
